...
static const int SGR_COLOR_RESET = 39;
...
{
	char buf[16];
	int len = snprintf(buf, sizeof(buf), "\033[1;%dm", color);
	if (len <= 0) {
		return;
	}
	size_t wlen = (size_t)((len < (int)sizeof(buf)) ? len : (int)sizeof(buf) - 1);
	do_write(tp->tty_minor, 0, KERNEL, (cp_grant_id_t) buf, wlen, CDEV_NONBLOCK, 0);
}

static void reset_color(tty_t *tp)
{
	char buf[16];
	int len = snprintf(buf, sizeof(buf), "\033[0;%dm", SGR_COLOR_RESET);
	if (len <= 0) {
		return;
	}
	size_t wlen = (size_t)((len < (int)sizeof(buf)) ? len : (int)sizeof(buf) - 1);
...
tty_t *line2tty(devminor_t line)
{
	...
	if (line >= CONS_MINOR && (line - CONS_MINOR) < NR_CONS) {
		...
		tp = tty_addr(NR_CONS + (line - RS232_MINOR));
	} else {
		return NULL;
	}
...
static void set_console_line(char term[CONS_ARG])
{
	...
	for (i = 1; i < NR_CONS; i++) {
		...
		if (!strncmp(term, cons,
		    CONS_ARG < (int)sizeof(cons) ? CONS_ARG - 1 : (int)sizeof(cons) - 1)) {
	...
	for (i = 0; i < NR_RS_LINES; i++) {
		...
		if (!strncmp(term, sercons,
		    CONS_ARG < (int)sizeof(sercons) ? CONS_ARG - 1 : (int)sizeof(sercons) - 1)) {
...
static void do_new_kmess(void)
{
	...
	if (bytes > 0) {
		...
		if (kernel_msg_color != 0) {
			set_color(tp, (int)kernel_msg_color);
		}
		do_write(tp->tty_minor, 0, KERNEL,
			(cp_grant_id_t) kernel_buf_copy, (size_t)bytes,
			CDEV_NONBLOCK, 0);
		if (kernel_msg_color != 0) {
			reset_color(tp);
		}
